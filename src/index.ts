...
const DEFAULTS = { enabled: true, size: 72, speed: 1, opacity: 1, autoWalk: true, idleSeconds: 8, x: 16, y: 120 };
const state = Object.assign({}, DEFAULTS, storage ?? {}) as typeof DEFAULTS;
...
function Cat() {
  ...
  const responder = React.useMemo(() => PanResponder.create({
    ...
    onPanResponderMove: (_, gesture) => {
      ...
      position.setValue({ x: clamp(dragOrigin.current.x + gesture.dx, 0, Math.max(0, width - state.size)), y: clamp(dragOrigin.current.y + gesture.dy, 0, Math.max(0, height - state.size - 20)) });
    },
  ...
  React.useEffect(() => {
    ...
    Animated.timing(position.x, { toValue: target, duration: Math.max(1800, 5000 / state.speed), useNativeDriver: false }).start(({ finished }: any) => {
      if (finished && !dragging.current) { state.x = target; save(); lastActivity.current = Date.now(); }
    });
  }, [mode]);

  const frame = FRAMES[mode][frameIndex % FRAMES[mode].length];
  return <Animated.View style={[styles.cat, { width: state.size, height: state.size + 24, opacity: state.opacity }, position.getLayout()]} {...responder.panHandlers}>
    <Pressable onPress={() => { lastActivity.current = Date.now(); setMode("sit"); }} onLongPress={() => { lastActivity.current = Date.now(); setMode("sleep"); }} style={{ alignItems: "center" }}>
...
function Settings() {
  ...
  const update = (key: keyof typeof DEFAULTS, value: any) => { (state as any)[key] = value; save(); redraw(v => v + 1); };
...
export default {
  name: "Floating Cat",
  description: "A small animated cat overlay for Discord.",
  authors: [{ name: "Scott" }],
  onLoad() {
